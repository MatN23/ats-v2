...
def check_torch() -> Tuple[bool, str]:
    ...
    cuda_str = f"+ CUDA {torch.version.cuda}" if torch.cuda.is_available() else "(CPU only, no CUDA detected)"
...
def check_flash_attention() -> Tuple[bool, Optional[str]]:
...
def check_config_memory(config_path: str) -> Tuple[bool, str]:
    ...
    if report.fits_on_single_gpu:
        return True, f"{config_path} estimated at {report.total_gb:.0f}GB, fits within {report.available_gb:.0f}GB budget"
    return False, (
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--config", default=None, help="Optional config to estimate memory for.")
    ...
    _print_line(ok, msg, is_warning=not ok)  # missing flash-attn is a soft warning, not fatal

    ok, msg = check_triton()
    _print_line(ok, msg, is_warning=not ok)  # missing triton is a soft warning, not fatal
    ...
    if any_hard_failure:
        print("\nSome required components are missing. See the ✗ lines above.", file=sys.stderr)
        return 1
    return 0
