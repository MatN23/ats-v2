...
| Triton kernels (`ats/model/*_triton.py`) | **Experimental; not on any execution path.** An audit (`tests/test_triton_audit.py`) established that no module under `ats/` imports any of these four modules — the only callers in the repository are tests — so they cannot run during training, evaluation, export or generation regardless of whether Triton is installed. They are also not usable for training as written: each Triton path launches a raw kernel into a `torch.empty()` buffer, which autograd cannot see, so gradients would stop at the call while the PyTorch fallback in the same function is differentiable. The Triton paths now raise rather than return a silently detached tensor. Wiring any of them in requires a `torch.autograd.Function` with a hand-written backward plus validation on real hardware; neither has been done. The PyTorch equivalents each duplicates are what actually runs |
