...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--tasks", default="mmlu,hellaswag,arc_easy",
        help="Comma-separated lm-eval-harness task names. Ignored if --config is given "
             "(perplexity mode).",
    )
    parser.add_argument("--batch-size", default="auto", help="lm-eval-harness batch size, or 'auto'.")
    parser.add_argument("--output-path", default="./eval_results", help="Where lm-eval-harness writes results.")
    parser.add_argument("--device", default="cuda", help="Device for lm-eval-harness (cuda, cpu, ...).")
    parser.add_argument(
        "--config", default=None,
...
def _resolve_config_path(checkpoint_dir: Path, explicit: Optional[str]) -> str:
...
def _ensure_hf_export(checkpoint_dir: Path, config_path: str, force: bool) -> Path:
    ...
    if config.model.use_mla or config.model.use_moe or config.model.use_mod \
            or config.model.use_mamba or config.model.model_type == "diffusion":
        raise ConfigError(
...
def _run_lm_eval(export_dir: Path, args: argparse.Namespace) -> int:
    command = [
        ...
        "--model", "hf",
        "--model_args", f"pretrained={export_dir},dtype=bfloat16",
        "--tasks", args.tasks,
...
def _run_perplexity_mode(config_path: str, checkpoint_dir: str, micro_batch_size_arg: Optional[int]) -> int:
    import torch
    ...
    logger.info(
        ...
        client_state["global_step"], client_state["epoch"],
    )
    model_engine.eval()

    eval_dataloader = build_dataloader(
        config.data, batch_size=micro_batch_size, rank=0, world_size=1, seed=config.training.seed,
    )
    total_loss = 0.0
    total_tokens = 0
    with torch.no_grad():
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.config is not None:
        try:
            return _run_perplexity_mode(args.config, args.checkpoint, args.micro_batch_size)
